...
def main() -> int:
    ...
    report = ScriptReport("tcesp_receitas_ingest")
    summary: dict[str, Any] = {"args": vars(args), "result": None, "errors": []}
    exit_code = 0

    report.emit("HERMES - Ingestao TCE-SP Receitas")
    ...
    try:
        ...
        from hermes.services.tcesp_ingestion import ingest_receitas

        if not args.dry_run and not all_tables_exist(engine, ["tcesp_municipios", "tcesp_receitas"]):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest TCE-SP receitas into HERMES database.")
