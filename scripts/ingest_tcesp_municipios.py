...
def main() -> int:
    ...
    report = ScriptReport("tcesp_municipios_ingest")
    summary: dict[str, Any] = {"args": vars(args), "result": None, "errors": []}
    exit_code = 0

    report.emit("HERMES - Ingestao TCE-SP Municipios")
    ...
    try:
        ...
        from hermes.services.tcesp_ingestion import ingest_municipios

        if not args.dry_run and not table_exists(engine, "tcesp_municipios"):
            message = "Tabela tcesp_municipios nao encontrada. Rode: docker compose run --rm api alembic upgrade head"
            report.emit(message)
            summary["errors"].append(message)
            exit_code = 3
            return exit_code

        result = ingest_municipios(dry_run=args.dry_run)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest TCE-SP municipios into HERMES database.")
